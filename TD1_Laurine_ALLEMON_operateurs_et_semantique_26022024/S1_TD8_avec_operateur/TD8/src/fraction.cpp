...
Fraction &Fraction::operator+=(Fraction const &f2)
{
    ...
    //on ne met pas *this = simplify(*this); car simplify ne ???
    //on ne met pas *this = simplify_itslef(*this); car simplify_itself ne retourne rien
    return *this;
}
...
bool operator<(Fraction const &f1, Fraction const &f2)
{
    simplify(f1); //est ce qu'il faut mettre une epserluette ici ?
    simplify(f2);
    return (f1.numerator < f2.numerator && f1.denominator < f2.denominator) || (f1.numerator > f2.numerator && f1.denominator > f1.numerator);
    //ou f1-f2<0
}
bool operator<=(Fraction const &f1, Fraction const &f2)
{
    return (f1 < f2) && ((f1.denominator == f2.denominator) && (f1.numerator == f2.numerator));
}
...
bool operator>=(Fraction const &f1, Fraction const &f2)
{
    return !(f1 < f2) && ((f1.denominator == f2.denominator) && (f1.numerator == f2.numerator)); //return !(f1 < f2) || (f1 == f2);
}
